...
<div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 mb-6">
    <div>
        <h2 class="text-xl font-semibold text-slate-900 dark:text-slate-100">Manajemen Kategori</h2>
        <p class="text-sm text-slate-500 dark:text-slate-400">Atur hierarki kategori produk</p>
    </div>
...
<div class="bg-white dark:bg-slate-900 rounded-2xl border border-slate-200 dark:border-slate-800 p-4 mb-4">
    <form class="flex flex-col sm:flex-row gap-3" method="GET">
        <div class="relative flex-1">
            ...
            <input type="text" name="q" placeholder="Cari kategori..." value="{{ request('q') }}"
        ...
        @if(request('q'))
            <a href="{{ route('categories.index') }}" class="px-4 py-2 text-slate-500 dark:text-slate-400 hover:text-red-600 text-sm rounded-xl border border-slate-200 dark:border-slate-700 transition text-center">Reset</a>
        @endif
    </form>
</div>
...
<div class="bg-white dark:bg-slate-900 rounded-2xl border border-slate-200 dark:border-slate-800 overflow-hidden">
        ...
        <table class="w-full text-sm text-left">
            <thead>
                <tr class="border-b border-slate-200 dark:border-slate-800 bg-slate-50 dark:bg-slate-800/50">
                    <th class="px-4 py-3 font-semibold text-slate-600 dark:text-slate-300">Nama Kategori</th>
                    <th class="px-4 py-3 font-semibold text-slate-600 dark:text-slate-300">Parent</th>
            ...
            <tbody>
                @forelse($categories as $category)
                    <tr class="border-b border-slate-100 dark:border-slate-800 hover:bg-slate-50 dark:hover:bg-slate-800/50 transition">
                        <td class="px-4 py-3 font-medium text-slate-900 dark:text-slate-100">
                            <div class="flex items-center gap-2">
                                @if($category->image_path)
                                    <img src="{{ asset('storage/' . $category->image_path) }}" class="w-8 h-8 rounded-lg object-cover" alt="">
                                @else
                                    <div class="w-8 h-8 rounded-lg bg-slate-100 dark:bg-slate-700 flex items-center justify-center">
                                        <iconify-icon icon="solar:sort-bold-duotone" class="text-slate-400"></iconify-icon>
                                    </div>
                                @endif
                                {{ $category->name }}
                            </div>
                        </td>
                        <td class="px-4 py-3 text-slate-500 dark:text-slate-400">{{ $category->parent?->name ?? '–' }}</td>
                        <td class="px-4 py-3">
                        ...
                        <td class="px-4 py-3 text-slate-600 dark:text-slate-400">{{ $category->products_count ?? 0 }} produk</td>
                        <td class="px-4 py-3 text-right">
                            <div class="flex items-center justify-end gap-2">
                                <a href="{{ route('categories.show', $category) }}" class="p-1.5 rounded-lg text-blue-600 dark:text-blue-400 hover:bg-blue-50 dark:hover:bg-blue-500/10 transition" title="Detail">
                                    <iconify-icon icon="solar:eye-bold" class="text-base"></iconify-icon>
                                </a>
                                <a href="{{ route('categories.edit', $category) }}" class="p-1.5 rounded-lg text-amber-600 dark:text-amber-400 hover:bg-amber-50 dark:hover:bg-amber-500/10 transition" title="Edit">
                    ...
                    <tr>
                        <td colspan="5" class="px-4 py-12 text-center">
                            <div class="flex flex-col items-center gap-2 text-slate-400 dark:text-slate-600">
                                <iconify-icon icon="solar:sort-bold-duotone" class="text-4xl"></iconify-icon>
                                <p class="text-sm">Belum ada kategori. <a href="{{ route('categories.create') }}" class="text-red-600 dark:text-red-400 hover:underline">Tambah sekarang</a></p>
                            </div>
                        </td>
                    </tr>
                @endforelse
    ...
    <div class="px-4 py-3 border-t border-slate-200 dark:border-slate-800">{{ $categories->withQueryString()->links() }}</div>
    @endif
</div>
