...
<div class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-6">
    <div>
        <h2 class="text-2xl font-bold text-[#212529] dark:text-white">Manajemen Kategori</h2>
        <p class="text-sm text-[#6C757D] mt-0.5">Atur hierarki kategori produk Wastra</p>
    </div>
...
<div class="bg-white dark:bg-[#1E1E1E] rounded-xl p-4 border border-[#E9ECEF] dark:border-[#334155] mb-4 shadow-sm">
    <form class="flex flex-wrap gap-3 items-center" method="GET">
        ...
        @if(request('q'))
            <a href="{{ route('categories.index') }}" class="px-4 py-2 border border-[#E9ECEF] dark:border-[#334155] text-[#6C757D] hover:text-[#fc1919] rounded-lg text-sm transition-colors">Reset</a>
        @endif
    </form>
</div>
        ...
        <table class="w-full text-left border-collapse min-w-[600px]">
            <thead>
                <tr class="bg-[#F8F9FA] dark:bg-[#121212] text-xs uppercase tracking-wide text-[#6C757D] border-b border-[#E9ECEF] dark:border-[#334155]">
                    <th class="py-4 px-5 font-semibold">Nama Kategori</th>
                    <th class="py-4 px-5 font-semibold">Parent</th>
                        ...
                        <td class="py-4 px-5">
                            <div class="flex items-center gap-3">
                                <div class="w-9 h-9 rounded-lg bg-gray-100 dark:bg-gray-800 flex items-center justify-center flex-shrink-0">
                                    <iconify-icon icon="solar:sort-bold" class="text-[#6C757D]"></iconify-icon>
                                </div>
                                <div>
                                    <p class="font-bold text-sm text-[#212529] dark:text-white">{{ $category->name }}</p>
                                    <p class="text-xs text-[#6C757D]">{{ $category->slug }}</p>
                                </div>
                            </div>
                        </td>
                        <td class="py-4 px-5 text-sm text-[#495057] dark:text-gray-400">{{ $category->parent?->name ?? '–' }}</td>
                        <td class="py-4 px-5 text-center">
                            <span class="{{ $category->is_active ? 'badge-success' : 'badge-danger' }} px-3 py-1 rounded-full text-xs font-semibold inline-block">
                                {{ $category->is_active ? 'Aktif' : 'Nonaktif' }}
                            </span>
                        </td>
                        <td class="py-4 px-5 text-center font-mono font-bold text-[#212529] dark:text-white text-sm">{{ $category->products_count ?? 0 }}</td>
                    ...
                    <tr>
                        <td colspan="5" class="py-12 text-center">
                            <div class="flex flex-col items-center gap-2 text-[#6C757D]">
                                <iconify-icon icon="solar:sort-bold-duotone" class="text-5xl opacity-30"></iconify-icon>
                                <p class="text-sm">Belum ada kategori. <a href="{{ route('categories.create') }}" class="text-[#fc1919] hover:underline font-medium">Tambah sekarang →</a></p>
                            </div>
                        </td>
                    </tr>
                @endforelse
